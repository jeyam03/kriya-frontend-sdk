var Pattern = {
  ...
  other: function (
    canvas,
    { size, spacing, lineWidth, backgroundColor, foregroundColor } = {}
  ) {
    var lineDrawer = Pattern.drawers.line(lineWidth);

    var drawThreeLines = function (
      context,
      x,
      y,
      length,
  ...
  asanoha: function (
    canvas,
    { size, lineDrawer, backgroundColor, foregroundColor } = {}
  ) {
  ...
  shippou: function (
    canvas,
    { lineWidth, radius, backgroundColor, foregroundColor } = {}
  ) {
  ...
  seigaiha: function (
    ...
    for (
      var offsetY = 0;
      offsetY < canvas.height + radius;
      offsetY = offsetY + radius
    ) {
      row++;
      column = 0;
      for (
        var offsetX = 0 + radius;
        offsetX < canvas.width + radius;
        offsetX = offsetX + radius * 2
      ) {
        column++;
  ...
  sayagata: function (
    canvas,
    { featureLength, lineWidth, angle, backgroundColor, foregroundColor } = {}
  ) {
    if (angle === undefined) {
      angle = Math.PI / 3;
    }
  ...
  stars: function (
    canvas,
    { featureLength, ratio, foregroundColor, backgroundColor } = {}
  ) {
  ...
  triangles: function (
    canvas,
    { featureLength, spacing, backgroundColor, foregroundColor } = {}
  ) {
  ...
  pattern003: function (
    canvas,
    { featureLength, lineWidth, spacing, backgroundColor, foregroundColor } = {}
  ) {
  ...
  gosper: function (
    canvas,
    { featureLength, foregroundColor, backgroundColor, lineWidth } = {}
  ) {
    var angle = Math.PI / 3;
    ...
    gosperOp = function (op) {
      if (op === "A") {
        return function (path, order, size) {
          drawGosper(path, order, size, true);
        };
      }
      if (op === "B") {
        return function (path, order, size) {
          drawGosper(path, order, size, false);
        };
      }
      if (op === "-") {
        return function (path, order, size) {
    ...
    order =
      Math.ceil(
        Pattern.logbase(
          3,
          ((maxDimension / 4) * 6) / featureLength / approximateScaleFactor
        )
      ) + 1;
  ...
  tiles: function (
    ...
    Pattern.alternate(
      ...
      canvas.width,
      canvas.height,
      featureWidth + lineWidth,
      featureHeight + verticalSpacing,
      alternate,
      function (x, y, offset) {
        var path = new Pattern.Path(context);
        image.add(path);
  ...
  dots: function (
    canvas,
    { foregroundColor, backgroundColor, radius, spacing, alternate } = {}
  ) {
